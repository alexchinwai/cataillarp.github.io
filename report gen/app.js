...
function parseInsuranceSection(sectionText, data) {
    ...
    for (let i = 0; i < headerPositions.length; i++) {
        ...
        const segmentText = sectionText.substring(startPos, endPos).trim()
            .replace(/^[︰:：\s]+/, ''); // Strip leading colons/punctuation
        ...
        if (segmentText.match(/^\s*\d+[\.．]/m)) {
            ...
            // Normalize: prepend newline if segment starts with "1." so split regex works uniformly
            const normalizedText = segmentText.replace(/^(\s*\d+[\.．])/, '\n$1');
            const policyBlocks = normalizedText.split(/\n\s*\d+[\.．]\s*/).filter(b => b.trim());
...
function parsePolicyBlock(lines, type, data) {
    ...
    // Remove list markers if any remain (e.g. "i)", "1.", "iv.")
    nameLine = nameLine.replace(/^[iIvVxX\d]+[\)\.．]\s*/, '');
    // Also handle simple "1. " or "2. " at start (more specific pattern)
    nameLine = nameLine.replace(/^\d+\.\s+/, '');

    // For lines with COMMAS and COLONS (standard format, not CSV):
    // The NAME is typically the FIRST comma-separated segment.
    // e.g. "HSBC 年金,原本價值：1,200,000 HKD ,月派：$5200,共240個月" -> Name = "HSBC 年金"
    if (nameLine.includes(',') || nameLine.includes('，')) {
        const segments = nameLine.split(/,|，/);
        // Take only the first segment as the name
        nameLine = segments[0].trim();
        // Remove numbered prefix again in case it was before the comma
        nameLine = nameLine.replace(/^\d+\.\s*/, '');
    }

    // Remove common attribute patterns from name line to avoid "Name: Plan A Coverage: 1000"
    // Regex removes: "保額:...", "保費:...", "Premium:...", "Cover:...", "原本價值:...", including currency suffix like "HKD"
    // Now allows COMMA as separator too: "年供, $20000"
    nameLine = nameLine.replace(/(?:保額|coverage|sum insured|現金價值|cash value|原本價值|原值)[^：:︰\d]*[：:︰,]\s*\$?[\d,]+(?:\s*[A-Z]{3})?/gi, '')
